...
void SSD1306::horizontal_scroll(bool right, uint8_t start, uint8_t stop, uint8_t speed)
{
	uint8_t buffer[9];
	buffer[0] = 0x00;
	if (right)
		buffer[1] = SSD1306_RIGHT_HORIZONTAL_SCROLL;
	else
		buffer[1] = SSD1306_LEFT_HORIZONTAL_SCROLL;
	buffer[2] = 0x00;
	buffer[3] = start;
	buffer[4] = speed;
	buffer[5] = stop;
	buffer[6] = 0x00;
	buffer[7] = 0xFF;
	buffer[8] = SSD1306_ACTIVATE_SCROLL;

	i2c_write_blocking(this->i2c, this->DevAddr, buffer, 9, false);
}


void SSD1306::vertical_horizontal_scroll(bool right, uint8_t start, uint8_t stop, uint8_t speed, uint8_t vert)
{
	uint8_t buffer[8];
	buffer[0] = 0x00;
	if (right)
		buffer[1] = SSD1306_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL;
	else
		buffer[1] = SSD1306_VERTICAL_AND_LEFT_HORIZONTAL_SCROLL;
	buffer[2] = 0x00;
	buffer[3] = start;
	buffer[4] = speed;
	buffer[5] = stop;
	buffer[6] = vert;
	buffer[7] = SSD1306_ACTIVATE_SCROLL;
